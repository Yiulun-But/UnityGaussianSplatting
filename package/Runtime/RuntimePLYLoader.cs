...
namespace GaussianSplatting.Runtime
{
    public class RuntimePLYLoader : MonoBehaviour
    {
        ...
        public GaussianSplatAsset LoadPLYFile(string filePath)
        {
            Debug.Log("Loading PLY file: " + filePath);
            ...
            GaussianFileReader.ReadFile(filePath, out splatData);

            if (!splatData.IsCreated || splatData.Length == 0)
            {
                Debug.LogError("Failed to read PLY file or no data found");
                return null;
            }

            Debug.Log($"Loaded {splatData.Length} splats from PLY file");

            var splatAsset = createSplatAsset(splatData, filePath);
        ...
        unsafe GaussianSplatAsset createSplatAsset(NativeArray<InputSplatData> inputSplats, string plyFilePath)
        {
            ...
            string baseName = Path.GetFileNameWithoutExtension(plyFilePath);

            GaussianSplatAsset asset = ScriptableObject.CreateInstance<GaussianSplatAsset>();
            GaussianSplatAsset.CameraInfo[] cameras = null;
            asset.Initialize(inputSplats.Length, m_FormatPos, m_FormatScale, m_FormatColor, m_FormatSH, boundsMin, boundsMax, cameras);
            asset.name = $"runtime_{baseName}";

            var dataHash = new Hash128((uint)asset.splatCount, (uint)asset.formatVersion, 0, 0);
            var chunkAsset = CreateChunkData(inputSplats, ref dataHash);
            var posAsset = CreatePositionsData(inputSplats, ref dataHash);
            var otherAsset = CreateOtherData(inputSplats, ref dataHash, default);
            var colorAsset = CreateColorData(inputSplats, ref dataHash);
            var shAsset = CreateSHData(inputSplats, ref dataHash, default);
            asset.SetDataHash(dataHash);
            asset.SetAssetFiles(chunkAsset, posAsset, otherAsset, colorAsset, shAsset);
        ...
        static TextAsset CreateChunkData(NativeArray<InputSplatData> splatData, ref Hash128 dataHash)
        {
            ...
            var chunkBytes = job.chunks
                .Reinterpret<byte>(UnsafeUtility.SizeOf<GaussianSplatAsset.ChunkInfo>())
                .ToArray();

            var textAsset = new TextAsset(chunkBytes);
            job.chunks.Dispose();
            return textAsset;
        }
        ...
        TextAsset CreatePositionsData(NativeArray<InputSplatData> inputSplats, ref Hash128 dataHash)
        {
            ...
            var textAsset = new TextAsset(data);

            data.Dispose();
            return textAsset;
        }

        TextAsset CreateOtherData(NativeArray<InputSplatData> inputSplats, ref Hash128 dataHash, NativeArray<int> splatSHIndices)
        {
        ...
        TextAsset CreateColorData(NativeArray<InputSplatData> inputSplats, ref Hash128 dataHash)
        {
            ...
            //     var textAsset = new TextAsset(cmpData);
            ...
            // {
                ...
                var textAsset = new TextAsset(jobConvert.outputData);
        ...
        static TextAsset EmitSimpleDataFile<T>(NativeArray<T> data, ref Hash128 dataHash) where T : unmanaged
        {
            dataHash.Append(data);
            var textAsset = new TextAsset(data.Reinterpret<byte>(UnsafeUtility.SizeOf<T>()));
            return textAsset;
        }

        TextAsset CreateSHData(NativeArray<InputSplatData> inputSplats, ref Hash128 dataHash, NativeArray<GaussianSplatAsset.SHTableItemFloat16> clusteredSHs)
        {
        ...
        {
            // No progress UI at runtime; always continue
            return true;
        }
